...
def build_examples():
    pcl_rows = read_fie_rows('../dataset/dontpatronizeme_pcl.tsv')
    ...
    for curr_row in pcl_rows:
        if int(curr_row[-1]) == 0:
            ...
            positive_examples.append(curr_row[-2])
...
for curr_train, curr_test in kfold_split:
    ...
    model = get_model()
    # training the model
    history = model.fit(train_examples[curr_train], train_labels[curr_train], batch_size=32, epochs=15)
    # model.save('./model')
    # getting the evaluation results
    test_loss, test_acc = model.evaluate(train_examples[curr_test], train_labels[curr_test])
    # printing the loss and accuracy
    print(f'Step {step}: Loss - {test_loss}, Accuracy - {test_acc}')
    total_accuracy += test_acc

total_accuracy /= n_splits
